...
const Codevorod = () => {
    ...
    const Buttonhandler =event=>{
        ...
        axios.post("http://185.105.239.124/api/user/login",phone)
        .then((response)=> {
    ...
    const Submithandler = (event)=>{
        ...
        {    console.log(Code)
            ...
            axios.post("http://185.105.239.124/api/user/checkOtp",Code)
            .then((response)=> {
                ...
                if (response) {
                    ...
                    localStorage.setItem('Lahzeusertoken', JSON.stringify(response.data.accessToken))
                    setTimeout(()=>Navigate("/cardnumvar"), 1000)
                }
            })
            .catch((errors)=> {
                console.log(errors);
